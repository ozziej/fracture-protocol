...
- A corrected Kenney-only visual integration slice is in place. Blender assembles reusable GLBs while preserving Kenney's original per-face white, warm orange, grey, charcoal, peach-rock, and green-crystal materials. Godot keeps gameplay indicators separate and adds only a small faction marker to each unit or building. The battlefield uses a neutral charcoal palette rather than blue ground and lighting. The Ranger is a compact hover scout; the Command Hub is a modular small-hangar complex with an adjacent satellite dish; the Resource Processor uses a round hangar; the Tech Centre uses `machine_wireless`; and Storage Silos use the barrel family. Resource fields scatter `rock_crystalsLargeA/B` clusters that disappear with depletion, while authored `rock_largeA/B` formations provide background scenery that remains hidden in undiscovered fog. The component/license record lives in `data/art_asset_manifest.json`; Poly Pizza and other mixed-style sources remain intentionally excluded.
...
- The first SFX pass now uses the supplied laser, damage, destruction, impact, mining, repair, hover, and UI-confirmation sounds through a presentation-only audio manager. Master, Effects, and Music buses are exposed in the pause menu; music remains optional until tracks are added under `res://audio/music`.
